...
folder are mapped to the application's root namespace, 'VistA.Surgery'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "VistA.Surgery"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'VistA.Surgery' in the same way
as `"VistA.Surgery/sass/src"`.
